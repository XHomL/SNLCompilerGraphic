...
/**
  ...
  *-----------------------------------------------
  * 预测分析表的构造过程
  * 1.求每个变量的FIRST集和FOLLOW集 -> 每个候选式的SELECT集
  * 2.检查是不是LL(1)文法。若是，则构造预测分析表。
  * FIRST(X)：可以从X推导出的所有串首终结符构成的集合
  * FOLLOW(X)：可能在某个句型中紧跟在A后边的终结符a的集合
  * IF 产生式右部的首部是终结符，产生式的SELECT集为该终结符
  * ELSE IF 当产生式的右部是空时，产生式的SELECT集为FOLLOW(X)
  * ELSE 产生式的SELECT集为FIRST(X)
  *------------------------------------------------
  * 预测分析法的工作过程
  * 从文法开始符号出发，在每一步推导过程中根据当前句型的最左非终结符A和当前输入符号a
  * 选择正确的A-产生式。为保证分析的确定性，选出的候选是必须是唯一的。
  *-------------------------------------------------
  * 预测分析中的错误检测
  * 1.栈顶的终结符和当前输入符号不匹配
  * 2.栈顶的非终结符与当前输入符号在预测分析表对应项中的信息为空
  *--------------------------------------------------
  * 预测分析中的错误恢复（恐慌模式）
  * 忽略输入中的一些符号，直到输入中出现由设计者选定的同步词法单元集合中的某个词法单元。
  * 该模式的效果依赖于同步集合的选取，一个简单例子如下：
  * 前提：当前输入符号为a，栈顶的符号为A，预测分析表为M，我们把FOLLOW(A)中的所有终结符放入非终结符A的同步记号集合（根据非终结符的FOLLOW集得到非终结符的同步词法单元）
  * ①A为栈顶的非终结符
  * 若M[A,a]为空，即非终结符A遇到a的产生式为空，根据恐慌模式，忽略输入符号a
  * 若M[A,a]为同步词法单元synch，则弹出A，试图继续分析A后面的语法成分。
  * ②A为栈顶的终结符
  * 若A不能匹配当前输入符号a，直接弹出A即可。
  *---------------------------------------------------
  * LL(1)文法
  * LL(1)文法是递归下降法中的特例，通过在输入中向前看固定个数（通常为1）符号来选择正确的A-产生式。
  *----------------------------------------------------
  * LL1的含义
  * 第一个L代表从左向右扫描输入符号串
  * 第二个L代表产生最左推导
...
/**
 * @brief LL1_parse::get_parsetree_head
 * @return 分析树的根结点
 */
...
void LL1_parse::run() {
	...
	//语法树节点栈syntaxtree_stack
	//child--指向子语法树节点的指针，为语法树节点指针类型，最多能有三个
	...
	while (!symbal_stack.empty()) {
		...
		if (NTSet.contains(symbal_stack.top())) {
			...
			//根据 非终结符ss、终结符head->getLex() 在预测分析表中找到相应的产生式id
			auto iter = table.find(QPair<LexType, LexType>(ss, head->getLex()));
			//迭代器不指向table最后一项的下一个“虚项”，说明找到了id
			if (table.cend() != iter) {
				//根据id选择相应的“产生式” ->不一样的操作
				process(iter.value());
			}
			else {
				//根据 非终结符ss、终结符head->getLex() 在预测分析表中找不到相应的产生式id
				//即发生了错误，在控制台输出错误信息，并根据恐慌模式，忽略ss，继续分析栈内的语法成分
				qDebug() << "i don't understand the :" << head->getLexName() << "in this line:" << lineno;
			}
		};
		//当符号栈顶元素是终结符
		if (TTSet.contains(symbal_stack.top())) {
			//栈顶元素ss 等于 当前输入符号，直接替换即可
			if (symbal_stack.top() == head->getLex()) {
				...
				//若ss不能匹配当前输入符号a，直接忽略ss即可，继续分析栈内的语法成分。
				qDebug() << "unexpected token:" << head->getLexName() << " in line " << lineno;
				head = head->next;
			}
...
void LL1_parse::process(int id)
{
	...
	case 63:
	{
		...
		//            current_treenode->kind.stmt=ReadK;
		(*t) = current_treenode;
		syntaxtree_stack.push(&current_treenode->sibling);
